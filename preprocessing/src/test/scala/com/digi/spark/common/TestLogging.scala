...
trait TestLogging extends BeforeAndAfterAll with BeforeAndAfterEach with TestLogger with TestSuiteMixin {
    ...
    override def beforeAll(): Unit = {
        ...
        logger.info(s"Full logs for test suite: ${getTestSuiteLogFile}")
        super.beforeAll
    }
    ...
    abstract override def withFixture(test: NoArgTest): Outcome = {
        ...
        def apply(): Outcome = {
            ...
            Try {
                ...
                Thread.sleep(1)
                logger.info(s"Full logs for test case: ${testCaseLogFile}")
            }
            try {
                test.apply
            } finally {
    ...
    def showLoggers(): Unit = {
        ...
        rootLogger.getLoggerRepository.getCurrentLoggers
            .foreach(log => {
                val log4jLogger = log.asInstanceOf[org.apache.log4j.Logger]
                logger.info(
